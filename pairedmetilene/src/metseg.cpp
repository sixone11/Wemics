...
//  double u = mannwhitney( la[0], l*noA, lb[0], l*noB, nonZeroPairs);   // by xp
//  p = mannwhitneyPvalue(u, l*noA, l*noB, nfo->MWU, MAXM, MAXN, *nonZeroPairs);  // by xp

  double * dif = (double *) ALLOCMEMORY(NULL, NULL, double, l*noA);  //by xp
   for(i=0; i< l*noA;i++){                       //by xp
	   dif[i] = la[0][i]-lb[0][i];          //by xp
     dif[i] = (dif[i]>0 ? dif[i] : -dif[i]);
 }	                                             //by xp
  p = LRFstat(dif, (size_t)l*noA);   // by xp
